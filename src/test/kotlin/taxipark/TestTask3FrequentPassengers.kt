...
class TestTask3FrequentPassengers {
    private fun testFrequentPassengers(driverIndex: Int, passengerIndexes: Set<Int>, tp: TaxiPark) {
        val message =
            "Wrong result for 'findFrequentPassengers()'. Driver: ${driver(driverIndex).name}." + tp.display()
        val expected = passengerIndexes.map { passenger(it) }.toSet()
        Assertions.assertEquals(expected, tp.findFrequentPassengers(driver(driverIndex)), message)
    }
