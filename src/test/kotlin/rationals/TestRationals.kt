...
class TestRationals {
    ...
    private fun testNormalizedForm(rational: String, normalizedForm: String) {
        Assertions.assertEquals(
            normalizedForm, rational.toRational().toString(), "Wrong normalized form for $rational"
        )
    }
    ...
    private fun testComparison(
        ...
        Assertions.assertEquals(
            expectedResult, result, "Wrong result for $first ${comparison.symbols} $second"
        )
    }
    ...
    private fun testInRange(element: String, start: String, end: String, expected: Boolean = true) {
        Assertions.assertEquals(
            expected,
            element.toRational() in start.toRational()..end.toRational(),
    ...
    fun test0Sample1Sum() {
        ...
        Assertions.assertEquals(5 divBy 6, sum, "Wrong result for sum")
    }

    @Test
    fun test0Sample2Difference() {
        val difference: Rational = (1 divBy 2) - (1 divBy 3)
        Assertions.assertEquals(1 divBy 6, difference, "Wrong result for difference")
    }

    @Test
    fun test0Sample3Product() {
        val product: Rational = (1 divBy 2) * (1 divBy 3)
        Assertions.assertEquals(1 divBy 6, product, "Wrong result for product")
    }

    @Test
    fun test0Sample4Quotient() {
        val quotient: Rational = (1 divBy 2) / (1 divBy 3)
        Assertions.assertEquals(3 divBy 2, quotient, "Wrong result for quotient")
    }

    @Test
    fun test0Sample5Negation() {
        val negation: Rational = -(1 divBy 2)
        Assertions.assertEquals(-1 divBy 2, negation, "Wrong result for negation")
    }
    ...
    fun test0Sample8Comparison() {
        Assertions.assertTrue((1 divBy 2) < (2 divBy 3), "Wrong result for comparison")
    }

    @Test
    fun test0Sample9InRange() {
        Assertions.assertTrue(
            (1 divBy 2) in (1 divBy 3)..(2 divBy 3), "Wrong result for checking belonging to a range"
        )
    }
    ...
    @Test
    fun test3Comparison1() =
        testComparison("20325830850349869048604856908", MORE, "-9192901948302584358938698", true)
