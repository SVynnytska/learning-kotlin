...
class TestGame2048Initializer : AbstractTestGameWithSmallNumbers() {
    ...
    fun test0() {
        ...
        Assertions.assertNull(nextValue, "$prefix new elements can't be added to a full board")
    }
    ...
    private fun testNextValue(input: String) {
        ...
        Assertions.assertTrue(cell in empty, "$prefix a value might be added only to one of the empty cells")
        Assertions.assertTrue(value in setOf(2, 4), "$prefix new element can only be 2 or 4")
    }
}
