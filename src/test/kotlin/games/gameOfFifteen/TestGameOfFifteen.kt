...
class TestGameOfFifteen {
    ...
    private fun testGame(initialPermutation: List<Int>, moves: List<Move>) {
        ...
        for ((index, move) in moves.withIndex()) {
            ...
            Assertions.assertTrue(game.canMove(), "The move for game of fifteen should be always possible")
            game.processMove(move.direction)
            val prev = moves[index - 1].board
            Assertions.assertEquals(
                move.board, game.asString(), "Wrong result after pressing ${move.direction} " +
                        "for\n$prev\n"
            )
        }
    }
